Pushbox.Level63 = function(game){
  ...
  this.points = {
    ...
    //the map
    map = game.add.tilemap('map3', 60, 60);
    map.addTilesetImage('tileset');
    layer = map.createLayer(0, game.world.width, game.world.height);
    layer = map.createLayer(0);
    layer.resizeWorld();
    map.setCollisionBetween(0,8);
    map.setTileIndexCallback(4,this.unlock,this);
    map.setTileLocationCallback(2, 0, 1, 1,this.unlock, this);

    //spots or lalagyan ng box
    this.spot = this.add.physicsGroup();
    var spot1 = new SpotPlatform(this.game,game.world.width-90, game.world.height-660, 'spots', this.spot);
    var spot2 = new SpotPlatform(this.game,game.world.width-90, game.world.height-120, 'spots', this.spot);

    
    //box
    box1 = new BoxPlatform(this.game, game.world.width-450, game.world.height-480, 'box');
    game.add.existing(box1);
    box2 = new BoxPlatform(this.game, game.world.width-450, game.world.height-300, 'box');
    game.add.existing(box2);
    this.blocked = {up: true, down:true, left: true, right: true};
    
    lock1 = game.add.sprite(game.world.width-240,game.world.height-118,'lock1');
    lock1.enableBody = true;
    game.physics.arcade.enable(lock1);
    lock1.body.setSize (56,56,0,0);
    lock1.body.immovable = true;

    lock2 = game.add.sprite(game.world.width-240,game.world.height-658,'lock4');
    ...
    key1 = new KeyPlatform(this.game,game.world.width-220, game.world.height -300, 'greenkey', this.key);
    key2 = new KeyPlatform(this.game,game.world.width-120, game.world.height -480, 'redkey', this.key);

      //  Lives
    lives = game.add.group();
    game.add.text(game.world.width - 1000, 10, 'Lives : ', { font: '34px Arial', fill: '#fff' });
