Pushbox.Level4_1 = function(game){

	   this.bmd = null;
  // points arrays - one for x and one for y
  this.points = {
    'x': [360,600,600,360,360],
    'y': [60,60,300,300,59]
};
};

    Pushbox.Level4_1.prototype = {
    ...
    //the map
    map = game.add.tilemap('map4', 60, 60);
    ...
    box2 = new BoxPlatform(this.game, game.world.width-480, game.world.height-487, 'box');
    game.add.existing(box2);
    
    //key
    this.key = this.add.physicsGroup();
    key1 = new KeyPlatform(this.game,game.world.width-600, game.world.height -480, 'yellowkey', this.key);
    key2 = new KeyPlatform(this.game,game.world.width-420, game.world.height -640, 'redkey', this.key);

    //lock
    lock1 = game.add.sprite(game.world.width-600,game.world.height-208,'lock1');
    lock1.enableBody = true;
    game.physics.arcade.enable(lock1);
    lock1.body.setSize (56,56,0,0);
    lock1.body.immovable = true;

    lock2 = game.add.sprite(game.world.width-838,game.world.height-658,'lock2');
    ...
    //setup our player
    player = game.add.sprite(game.world.width-360, game.world.height - 300, 'boy'); //create and position player
